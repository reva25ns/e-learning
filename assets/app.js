...
function prepareIndonesianVoice(){
 ...
 const idVoices=voices.filter(v=>/^id(?:[-_]|$)/i.test(v.lang));
 // Prefer a native Indonesian voice. On iPhone the available voice names differ
 // from Android, so never depend on a specific provider/name.
 indonesianVoice=
  idVoices.find(v=>/indonesia|indonesian/i.test(v.name))||
  idVoices.find(v=>v.localService)||
  idVoices[0]||
  voices.find(v=>/indonesia|indonesian/i.test(v.name))||null;
}
...
function speak(t){
 ...
 const text=normalizeSpeechText(t);
 prepareIndonesianVoice();
 const synth=window.speechSynthesis;
 synth.cancel();
 const u=new SpeechSynthesisUtterance(text);
 const resumeSong=Boolean(childrenSong&&!childrenSong.paused);
 if(resumeSong)childrenSong.volume=.08;
 u.lang='id-ID';
 if(indonesianVoice)u.voice=indonesianVoice;
 u.volume=1;
 u.rate=.95;
 u.pitch=1.05;
 u.onend=u.onerror=()=>{if(resumeSong&&childrenSong&&!childrenSong.paused)childrenSong.volume=.3};
 // iOS Safari can receive the utterance before its voice list is ready.
 setTimeout(()=>{
  if(!S.sound)return;
  prepareIndonesianVoice();
  if(indonesianVoice)u.voice=indonesianVoice;
...
$('continue').onclick=()=>{let n=$('nameInput').value.trim();if(!n){$('nameErr').textContent='Tulis nama kamu dulu ya 😊';return}S.name=n;$('hello').innerHTML=`Halo, ${esc(n)}! 👋`;show('menu');speak(`Halo ${n}! Sebelum belajar, jangan lupa membaca doa.`)};
...
function start(g){
 ...
 S.numberOrder=g==='berhitung'?[...Array(nums.length).keys()]:[];
...
function speakColorWarmupSequence(onDone){
 ...
 const next=()=>{
  ...
  const u=new SpeechSynthesisUtterance(normalizeSpeechText(lines[index++]));
  u.lang='id-ID';
  if(indonesianVoice)u.voice=indonesianVoice;
  u.volume=1;u.rate=.9;u.pitch=1.05;
